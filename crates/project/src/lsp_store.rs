    ...
    pub fn resolve_inlay_hint(
        ...
        if let Some((upstream_client, project_id)) = self.upstream_client() {
            ...
            cx.spawn(async move |_this, cx| {
                ...
                eprintln!("inlayHint/resolve response: {:#?}", resolved_hint);
                ...
                if needs_fallback {
                    eprintln!("Tooltip is None, attempting fallback hover fetch");
                        ...
                        for part in parts.iter_mut() {
                            if part.tooltip.is_none() {
                                if let Some(location) = &part.location {
                                    eprintln!("Fetching hover for location: {:?}", location);
                                    ...
                                    if let Ok(hover_response) = lang_server
                                        ...
                                        .into_response()
                                    {
                                        eprintln!("Hover response: {:#?}", hover_response);
